	...
	testCases := []struct {
		...
		},
		{
			...
			events: []tcell.Event{
				...
				tcell.NewEventKey(tcell.KeyRune, "e", tcell.ModNone),
				tcell.NewEventKey(tcell.KeyBackspace, "\x7f", tcell.ModNone),
				tcell.NewEventKey(tcell.KeyBackspace, "\x7f", tcell.ModNone),
			},
			expectedCursorPos: 8,
			expectedText:      "Lorem ipsum dolor\nsit amet consectetur\nadipiscing elit",
		...
		},
		{
			...
			events: []tcell.Event{
				tcell.NewEventKey(tcell.KeyRune, "i", tcell.ModNone),
				tcell.NewEventKey(tcell.KeyRune, string(utf8.MaxRune), tcell.ModNone),
			},
			expectedCursorPos: 1,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			inputEvents := []tcell.Event{
				tcell.NewEventKey(tcell.KeyRune, string(tc.enterVisualModeKey), tcell.ModNone), // enter visual mode
				tcell.NewEventKey(tcell.KeyEsc, "", tcell.ModNone),                 // exit visual mode
...
func inputEventsForBracketedPaste(s string) []tcell.Event {
	...
	for _, r := range s {
		if r == '\n' {
			inputEvents = append(inputEvents, tcell.NewEventKey(tcell.KeyEnter, "", tcell.ModNone))
		} else if r == '\t' {
			inputEvents = append(inputEvents, tcell.NewEventKey(tcell.KeyTab, "", tcell.ModNone))
		} else {
			inputEvents = append(inputEvents, tcell.NewEventKey(tcell.KeyRune, string(r), tcell.ModNone))
		}
	}
...
func TestBracketedPasteInUserMacro(t *testing.T) {
	...
	inputEvents = append(inputEvents, inputEventsForBracketedPaste("abc\n")...)
	inputEvents = append(inputEvents, tcell.NewEventKey(tcell.KeyEscape, "", tcell.ModNone))
...
func TestCountLimits(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc, func(t *testing.T) {
			...
			for _, r := range tc {
				event := tcell.NewEventKey(tcell.KeyRune, string(r), tcell.ModNone)
				inputCtx := ContextFromEditorState(editorState)
				action := interpreter.ProcessEvent(event, inputCtx)
				action(editorState)
			}
...
func TestTextFieldMode(t *testing.T) {
	...
	inputEvent(tcell.NewEventKey(tcell.KeyEnter, "", tcell.ModNone))
	...
	inputEvent(tcell.NewEventKey(tcell.KeyBackspace, "", tcell.ModNone))
	inputEvent(tcell.NewEventKey(tcell.KeyBackspace, "", tcell.ModNone))
	inputEvent(tcell.NewEventKey(tcell.KeyBackspace, "", tcell.ModNone))
