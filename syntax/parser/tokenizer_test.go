	...
	testCases := []struct {
		...
		},
		{
			...
			expectedTokens: []Token{
				...
				{StartPos: 3, EndPos: 4, LookaheadPos: 4, Role: TokenRoleNone},

				// "foo"
				{StartPos: 4, EndPos: 7, LookaheadPos: 8, Role: TokenRoleIdentifier},
				{StartPos: 7, EndPos: 15, LookaheadPos: 15, Role: TokenRoleNone},

				// "return"
				{StartPos: 15, EndPos: 21, LookaheadPos: 22, Role: TokenRoleKeyword},
				{StartPos: 21, EndPos: 22, LookaheadPos: 22, Role: TokenRoleNone},

				// "bar"
				{StartPos: 22, EndPos: 25, LookaheadPos: 26, Role: TokenRoleIdentifier},
				{StartPos: 25, EndPos: 26, LookaheadPos: 26, Role: TokenRoleNone},

				// "+"
				{StartPos: 26, EndPos: 27, LookaheadPos: 28, Role: TokenRoleOperator},
				{StartPos: 27, EndPos: 28, LookaheadPos: 28, Role: TokenRoleNone},
...
func TestRetokenizeAffectsEarlierEmptyToken(t *testing.T) {
	tokenizer, err := GenerateTokenizer([]TokenizerRule{
		{
			Regexp:    `"([^\"\n]|\\")*"`,
			TokenRole: TokenRoleString,
		},
		{
			Regexp:    `[a-zA-Z0-9]+`,
	...
	// Insert a double quote, completing the string starting at position zero.
	// The empty token starting at position zero should have lookahead at least to this position
	// because the tokenizer should have been looking for the end quote.
	initialText := `"abc        `
	updatedText := `"abc    "    `
	edit := Edit{Pos: 8, NumInserted: 1}

	assertRetokenizeMatchesFullTokenize(t, tokenizer, initialText, func(s string) (string, Edit) {
		return updatedText, edit
	})
}
